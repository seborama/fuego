...
func TestCollector_Collect_GroupingBy(t *testing.T) {
	stringLength := func(e T[EntryString]) R[EntryString] {
		return e.(EntryString).Len()
	}
	collectorWithNilFinisher := ToEntrySlice[EntryString]()
	collectorWithNilFinisher.finisher = nil

	strs := EntrySlice[EntryString]{}
	...
	assert.Equal(t, EntryMap[EntryString, EntryString]{}, got)
}
